...
static void *socket_thread_fn(void *arg) {
  ...
  if (sockfd < 0) {
	...
	while((n = read(sockfd,buffer,5)) != 0) {
		if(buffer[0] == 0) {
			new_freq = chars_to_int((const char *)buffer);
			// fm->freqs[0] = new_freq;
...
int main(int argc, char **argv)
{
	...
	fprintf(stderr, "Create UDP thread\n");
	pthread_create(&socket_freq, NULL, socket_thread_fn, (void *)(NULL));
	fprintf(stderr, "Created UDP thread\n");
